...
export default function DoctorDashboard() {
  ...
  const fetchExistingPrescriptions = async (patientId) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/doctor/prescriptions/${patientId}`,
        {
          headers: {
  ...
  const fetchPatientsWithAppointments = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:5000/api/doctor/patients-with-appointments",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          ...
          <CardContent>
            ...
            {appointments.length > 0 ? (
              <p className="text-xs text-gray-500">
                Next: {appointments[0].patientId.firstName}{" "}
  ...
  const renderPatientManagement = () => {
    ...
    const fetchAvailableSlots = async (patientId, date) => {
      ...
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(
          `http://localhost:5000/api/doctor/available-slots?patientId=${patientId}&date=${date}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    const handleDeletePrescription = async (prescriptionId) => {
      if (
        ...
        try {
          const token = localStorage.getItem("token");
          const response = await fetch(
            `http://localhost:5000/api/doctor/prescriptions/${prescriptionId}`,
            {
              method: "DELETE",
              headers: {
                Authorization: `Bearer ${token}`,
    ...
    const handleSubmit = async (e) => {
      ...
      if (selectedAction === "prescribe-medication") {
        try {
          ...
          if (response.ok) {
            ...
            alert(
              ...
              } medication: ${errorData.error}`
            );
          }
        } catch (error) {
          alert(
            `Error ${
              appointmentData.prescriptionId ? "updating" : "prescribing"
            } medication. Please try again.`
          );
        }
        ...
        try {
          const token = localStorage.getItem("token");
          const response = await fetch(
            "http://localhost:5000/api/doctor/schedule-appointment",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
