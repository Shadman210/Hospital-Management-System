...
router.post("/add-doctor", auth, async (req, res) => {
  ...
  try {
    // 1️⃣ Check if a user or doctor already exists with this email
    ...
    // 4️⃣ Create matching user record
    const user = new User({
      firstName,
      lastName,
      email,
    ...
    res
      .status(201)
      .send({ message: "Doctor and user account created successfully" });
  } catch (error) {
    console.error("Error adding doctor:", error);
    if (error.code === 11000) {
      return res
        .status(400)
        .send({ error: "Email or license number already exists" });
    }
